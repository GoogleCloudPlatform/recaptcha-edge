...
type calloutEvent = "requestHeaders" |
        "requestBody" |
        "requestTrailers" |
        "responseHeaders" |
        "responseBody" |
...
describe("WAF Callouts Suite", async function() {
    ...
    it.each([
        ...
        expect(processingResponse).toStrictEqual(create(ProcessingResponseSchema, {
            response: {
                case: event.type as calloutEvent,
                value: {}
            }
...
async function* genRequests(...requests: MessageInitShape<typeof ProcessingRequestSchema>[])
...
async function sendRequest(client, ...requests: MessageInitShape<typeof ProcessingRequestSchema>[]): Promise<ProcessingResponse[]> {
    let count = 0;
